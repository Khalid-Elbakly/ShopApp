        ...
        listener: (context,state) {
          if(state is RegisterSuccessState){
            if(state.Registermodel.status){
              ...
              CacheHelper.setData(key: 'token', value: state.Registermodel.data.token).then((value) => token = state.Registermodel.data.token);
              print(token);
              Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => HomeLayout()));
            } else {
              print(state.Registermodel.status);
              Fluttertoast.showToast(msg: state.Registermodel.message,backgroundColor: Colors.red);
            }
          }
        },
